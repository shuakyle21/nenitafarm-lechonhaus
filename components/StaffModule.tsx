        ...
        try {
            ...
            const tableData = reportData.map(record => {
                ...
                return [
                    ...
                    status === 'ABSENT' ? (record.notes || '-') : (record.clock_out ? new Date(record.clock_out).toLocaleTimeString() : 'Active'),
                    status,
                    `P ${wage.toFixed(2)}`,
                    `P ${pay.toFixed(2)}`
                ];
            ...
            autoTable(doc, {
                head: [['Date', 'Staff Name', 'Role', 'In', 'Out/Reason', 'Status', 'Daily Rate', 'Est. Pay']],
                body: tableData,
                startY: 45,
                theme: 'grid',
                styles: { fontSize: 8 },
            ...
            <div className="flex-1 overflow-y-auto p-8">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {staffList.map(staff => {
                        ...
                        return (
                            <div key={staff.id} className="bg-white rounded-2xl p-6 shadow-sm border border-stone-200 hover:shadow-md transition-all">
                                <div className="flex justify-between items-start mb-4">
                                    ...
                                    <div className={`w-3 h-3 rounded-full ${status === 'CLOCKED_IN' ? 'bg-green-500' :
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="p-4 border-b border-stone-100 flex justify-between items-center bg-stone-50">
                            ...
                            <button onClick={() => setIsAddModalOpen(false)} className="p-2 hover:bg-stone-200 rounded-full transition-colors">
                        ...
                        <form onSubmit={handleAddStaff} className="p-6 space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-bold text-stone-600 mb-1">Daily Wage (PHP)</label>
                                <input
                                    type="number"
                                    required
                                    min="0"
                            ...
                            <div>
                                <label className="block text-sm font-bold text-stone-600 mb-1">PIN (4 digits)</label>
                                <input
                                    type="text"
                                    required
                                    maxLength={4}
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
                    <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
                        <div className="p-4 border-b border-stone-100 flex justify-between items-center bg-red-50">
                            ...
                            <button onClick={() => setIsAbsentModalOpen(false)} className="p-2 hover:bg-red-100 rounded-full transition-colors">
                        ...
                        <form onSubmit={handleMarkAbsent} className="p-6 space-y-4">
                            <p className="text-sm text-stone-600">
                                Please provide a reason for this absence. This will be recorded in the attendance log.
                            </p>
                            <div>
                                <label className="block text-sm font-bold text-stone-600 mb-1">Reason for Absence</label>
                                <textarea
                                    required
